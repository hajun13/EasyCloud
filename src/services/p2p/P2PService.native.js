/**
 * P2P 서비스 모듈 - WebRTC와 소켓 연결 관리 (네이티브 환경용)
 */
...
class P2PService {
  constructor() {
    ...
    this.isConnected = this.isConnected.bind(this);  // 바인딩 추가
  }
   ...
   */
  sendMessage(data) {
    if (!this.isConnected() || !this.dataChannel || this.dataChannel.readyState !== 'open') {  // 메소드 호출로 변경
   ...
   */
  isConnected() {
    return this._isConnected && this.dataChannel && this.dataChannel.readyState === 'open';  // 변수명 변경
  }
